class Admin::GuestsController < Admin::BaseController
  ...
  def apply_filters(guests)
    ...
    if params[:rsvp_status].present?
      guests = guests.joins(:rsvp).where(rsvps: { status: params[:rsvp_status] })
    end
  ...
  def generate_csv(guests)
    CSV.generate(headers: true) do |csv|
      csv << ['First Name', 'Last Name', 'Email', 'Phone', 'Household', 'RSVP Status', 'Meal Choice', 'Dietary Restrictions']
